...
(define (eval-prim [op : Symbol] [args : (Listof Value)]) : Value
  (match (list op args)
    ...
    [(list '/ (list (NumV l) (NumV r)))
     (cond
       [(zero? r) (error "SHEQ: division by zero ~a/~a" l r)]
       [else (NumV (/ l r))])]
    ...
    [(list 'substring (list (StrV s) (NumV start) (NumV stop)))
     (cond
       [(or (not (exact-integer? start))
        ...
        (error "SHEQ: invalid substring range ~a ~a ~a" s start stop)]
...
(check-exn
 #px"SHEQ"
 (lambda () (top-interp '{substring "test" 3 1})))

;; Invalid substring start index (negative)
(check-exn
 #px"SHEQ"
 (lambda () (top-interp '{substring "test" -1 2})))

;; Non-integer substring index
(check-exn
 #px"SHEQ"
 (lambda () (top-interp '{substring "hello" 0.5 3})))
